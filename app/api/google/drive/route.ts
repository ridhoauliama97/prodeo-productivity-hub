...
export async function GET(req: NextRequest) {
  ...
  if (isExpired && tokenData.provider_refresh_token) {
    ...
    const newToken = await refreshAccessToken(
      user.id,
      tokenData.provider_refresh_token,
    );
    if (newToken) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        return NextResponse.json(
          {
            error:
              "Google session expired. Please reconnect your Google account in your Profile.",
            expired: true,
          },
          { status: 401 },
        );
      ...
      data.files = [
        {
          ...
          iconLink:
            "https://fonts.gstatic.com/s/i/productlogos/drive_shared_with_me/v6/web-512dp/logo_drive_shared_with_me_color_1x_web_512dp.png",
        },
        ...(data.files || []),
      ];
    }
