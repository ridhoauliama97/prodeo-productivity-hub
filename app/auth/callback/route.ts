...
export async function GET(request: NextRequest) {
  ...
  if (code) {
    ...
    if (data?.session) {
      ...
      console.log('Auth Callback: Refresh Token exists:', !!providerRefreshToken)
      ...
      if (providerToken && user) {
        try {
          ...
          if (upsertError) {
            console.error('Auth Callback: Error upserting to google_tokens:', upsertError)
          } else {
            console.log('Auth Callback: Google tokens stored successfully')
          }
        } catch (tokenError) {
          console.error('Auth Callback: Unexpected error storing Google tokens:', tokenError)
        }
      } else {
        console.warn('Auth Callback: Missing providerToken or user. Check Supabase OAuth settings.')
      }
    }
