...
export default function ChatPage() {
  ...
  const handleSendMessage = async (e: React.FormEvent, attachment?: { url: string, name: string }) => {
    e.preventDefault()
    ...
    try {
      const { error } = await supabase.from('messages').insert({
        ...
        content: content || (attachment ? `Sent an attachment: ${attachment.name}` : ''),
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await handleSendMessage({ preventDefault: () => {} } as any, { url: publicUrl, name: file.name })
  ...
  const handleClearConversation = async () => {
    try {
      ...
      if (selectedMember) {
        ...
        deleteQuery = deleteQuery.or(`and(sender_id.eq.${user1},receiver_id.eq.${user2}),and(sender_id.eq.${user2},receiver_id.eq.${user1})`)
      } else {
        // Group chat deletion
    ...
    <SidebarProvider>
              ...
              {selectedMember ? (
                <>
                  <Avatar className="h-8 w-8 ring-2 ring-primary/10">
                    <AvatarImage src={selectedMember.user_profiles?.avatar_url} />
                    <AvatarFallback>{selectedMember.user_profiles?.full_name?.charAt(0) || 'U'}</AvatarFallback>
                  </Avatar>
                  <div className="flex flex-col -space-y-0.5">
                    <span className="text-sm font-bold">{selectedMember.user_profiles?.full_name}</span>
                ...
                <>
                  ...
                  <div className="flex flex-col">
                    <span className="text-sm font-semibold">Workspace Chat</span>
                    <span className="text-[10px] text-muted-foreground">Team communication</span>
                  </div>
                </>
              )}
            </div>
          ...
          <div className="flex items-center gap-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuItem onClick={() => setIsMediaGalleryOpen(true)} className="cursor-pointer">
                  View All Media
                </DropdownMenuItem>
          ...
          <div className="w-72 border-r bg-muted/20 hidden md:flex flex-col">
            <ScrollArea className="flex-1">
              <div className="p-4 space-y-6">
                <div>
                  <h3 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-3 px-2">Channels</h3>
                  <Button 
                ...
                <div>
                  <h3 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-3 px-2">Members</h3>
                  <div className="space-y-1">
                    {members.filter(m => m.user_id !== user?.id).map((member) => {
                      ...
                      return (
                        <Button 
                          key={member.id}
                          variant={selectedMember?.id === member.id ? "secondary" : "ghost"} 
                        ...
                        >
                          <div className="relative shrink-0 flex items-center justify-center w-10">
                            ...
                            <div className={`absolute -right-0.5 -bottom-0.5 w-3.5 h-3.5 rounded-full ring-2 ring-background ${isOnline ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]' : 'bg-muted-foreground/40'}`} />
                          </div>
                          ...
                          <div className="flex flex-col items-start justify-center min-w-0 text-left">
                            ...
                            <div className={`text-[10px] font-extrabold uppercase tracking-widest mt-1 leading-none ${isOnline ? 'text-green-500' : 'text-muted-foreground/50'}`}>
              ...
              <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-8">
                {fetchingMessages ? (
                  <div className="flex flex-col items-center justify-center py-24 text-center animate-in fade-in duration-300">
                    ...
                    <span className="text-xs text-muted-foreground mt-3">Loading messages...</span>
                  </div>
                ) : messages.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-24 text-center animate-in fade-in zoom-in duration-500">
                    ...
                    <p className="text-sm text-muted-foreground max-w-[280px] mt-2 leading-relaxed">
                      This is the beginning of your conversation. Type something to get started!
                    </p>
                  </div>
                ) : null}
                <div className="space-y-6">
                  {messages.map((msg, i) => {
                    ...
                    return (
                      <div key={msg.id} className={`flex gap-4 group animate-in slide-in-from-bottom-2 duration-300 ${isMe ? 'flex-row-reverse' : ''}`}>
                        ...
                        <div className={`flex flex-col max-w-[80%] md:max-w-[65%] ${isMe ? 'items-end' : ''}`}>
                          {showAvatar && (
                            <div className={`flex items-center gap-2 mb-1.5 px-1 ${isMe ? 'flex-row-reverse' : ''}`}>
                              <span className="text-[11px] font-bold text-foreground/80">{msg.sender?.full_name}</span>
                              <div className="flex items-center gap-1">
                                <span className="text-[9px] text-muted-foreground font-medium">
                                  {format(new Date(msg.created_at), 'HH:mm')}
                                </span>
                                {isMe && (
                                  <CheckCheck className={`w-3 h-3 transition-colors ${msg.is_read ? 'text-sky-400' : 'text-muted-foreground/50'}`} />
                                )}
                              </div>
                            </div>
                          )}
                          <div className={`px-4 py-2.5 rounded-2xl text-[13px] leading-relaxed shadow-sm transition-all hover:shadow-md ${
                            isMe 
                              ...
                              <div className="mt-3 space-y-2">
                                {msg.attachment_name?.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                                  <div 
                                    ...
                                    onClick={() => setLightboxMedia({ url: msg.attachment_url, name: msg.attachment_name, type: 'image' })}
                                  >
                                    <img 
                                  ...
                                  <div className="relative rounded-xl overflow-hidden border bg-black group/media max-h-[450px] w-full flex items-center justify-center shadow-inner">
                                    ...
                                    <Button 
                                      ...
                                      onClick={() => setLightboxMedia({ url: msg.attachment_url, name: msg.attachment_name, type: 'video' })}
                                    >
                                      <Maximize2 className="h-3.5 w-3.5" />
                                    </Button>
                                  </div>
                                ) : (
                                  <div className={`p-3 rounded-xl border flex items-center gap-3 ${isMe ? 'bg-white/10 border-white/20' : 'bg-background/50 border-border'}`}>
                                    <div className={`p-2.5 rounded-lg ${isMe ? 'bg-white/20' : 'bg-primary/10'}`}>
                                      {msg.attachment_name?.endsWith('.pdf') ? <FileText className="w-5 h-5" /> : <FileIcon className="w-5 h-5" />}
                                    </div>
                                    <div className="flex flex-col overflow-hidden flex-1">
                                      <span className="text-[12px] font-bold truncate">{msg.attachment_name}</span>
                                      <a 
              ...
              >
                <div className="bg-muted/30 rounded-[24px] border shadow-sm transition-all focus-within:shadow-md focus-within:border-primary/50 focus-within:bg-background">
                  <div className="flex items-end gap-2 p-2">
                    ...
                    <textarea 
                      rows={1}
                      placeholder={selectedMember ? `Message @${selectedMember.user_profiles?.full_name}...` : "Message the group..."}
                    ...
                    <div className="flex items-center gap-1 pb-0.5 pr-1">
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button type="button" variant="ghost" size="icon" className="h-10 w-10 text-muted-foreground rounded-full hover:bg-primary/5 hover:text-primary transition-colors">
                            <Smile className="w-5 h-5" />
                          </Button>
                        </PopoverTrigger>
                        <PopoverContent className="w-80 p-0 bg-zinc-950 border-white/10 shadow-2xl" side="top" align="end" sideOffset={10}>
                          <div className="p-3">
                            <ScrollArea className="h-72">
                              <div className="space-y-4">
                                {COMMON_EMOJIS.map((category) => (
                                  <div key={category.label}>
                                    <h4 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-2 px-1">{category.label}</h4>
      ...
      {lightboxMedia && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-sm animate-in fade-in duration-300 flex flex-col">
          <div className="h-16 flex items-center justify-between px-6 text-white bg-gradient-to-b from-black/50 to-transparent">
            <div className="flex flex-col">
              <span className="text-sm font-bold truncate max-w-md">{lightboxMedia.name}</span>
              <span className="text-[10px] text-white/60">Full screen preview</span>
            </div>
            <div className="flex items-center gap-2">
              <Button 
                variant="ghost" 
                ...
                asChild
              >
                <a href={lightboxMedia.url} download={lightboxMedia.name} target="_blank" rel="noopener noreferrer">
        ...
        <DialogContent className="max-w-4xl max-h-[80vh] flex flex-col bg-zinc-950 border-white/10 text-white">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold flex items-center gap-2">
               ...
               <span className="text-xs font-normal text-muted-foreground ml-2">
                 {messages.filter(m => m.attachment_url && m.attachment_name?.match(/\.(jpg|jpeg|png|gif|webp|mp4|webm|ogg)$/i)).length} items
               </span>
            </DialogTitle>
          </DialogHeader>
          <ScrollArea className="flex-1 mt-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-1">
              {messages
                .filter(m => m.attachment_url && m.attachment_name?.match(/\.(jpg|jpeg|png|gif|webp|mp4|webm|ogg)$/i))
                .reverse()
                .map((msg) => (
                  <div 
                    key={msg.id} 
                    ...
                    onClick={() => {
                      ...
                      setLightboxMedia({
                        ...
                        type: msg.attachment_name?.match(/\.(mp4|webm|ogg)$/i) ? 'video' : 'image'
                      })
                    }}
                  >
                    {msg.attachment_name?.match(/\.(mp4|webm|ogg)$/i) ? (
            ...
            {messages.filter(m => m.attachment_url && m.attachment_name?.match(/\.(jpg|jpeg|png|gif|webp|mp4|webm|ogg)$/i)).length === 0 && (
