...
export default function EmailPage() {
  ...
  const [composeAttachment, setComposeAttachment] = useState<{ url: string, name: string } | null>(null)
  ...
  const fetchEmails = async (folder: string) => {
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('Emails table does not exist yet. Please run the migration.')
          return
        }
        throw error
      }
    ...
    <SidebarProvider>
      ...
      <SidebarInset className="flex flex-col h-screen overflow-hidden bg-background">
        ...
        <div className="flex-1 flex overflow-hidden">
            ...
            <ScrollArea className="flex-1">
              <div className="px-3 py-2 space-y-1">
                ...
                >
                  ...
                  <span className="text-[10px] font-bold bg-primary/10 px-1.5 py-0.5 rounded-full">12</span>
                </Button>
                <Button 
              ...
              <div className="mt-8 px-5">
                <h3 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-4">Labels</h3>
          ...
          {/* Email List */}
          <div className={`${selectedEmail ? 'hidden xl:flex' : 'flex'} w-full lg:w-[400px] border-r flex flex-col bg-muted/5`}>
              ...
              <div className="divide-y divide-muted/50">
                {emails.length === 0 && (
                   <div className="p-8 text-center">
                     <p className="text-sm text-muted-foreground">No emails found in this folder.</p>
                   </div>
                )}
                {emails.map((email) => (
                  <div 
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <span className={`text-sm truncate ${!email.is_read ? 'font-bold text-foreground' : 'font-medium text-muted-foreground'}`}>
                        ...
                        <h4 className={`text-xs truncate mb-1 ${!email.is_read ? 'font-bold text-foreground' : 'font-semibold text-muted-foreground/80'}`}>
                          {email.subject}
                        </h4>
          ...
          {/* Email Preview */}
          <div className={`flex-1 flex flex-col bg-background relative ${!selectedEmail ? 'hidden lg:flex items-center justify-center' : 'flex'}`}>
            {selectedEmail ? (
              <>
                <header className="h-16 border-b flex items-center justify-between px-6 sticky top-0 bg-background/95 backdrop-blur z-10">
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" className="rounded-full lg:hidden mr-2" onClick={() => setSelectedEmail(null)}>
                    ...
                    <Button variant="ghost" size="icon" className="rounded-full text-muted-foreground hover:text-destructive hover:bg-destructive/5 transition-colors">
                      <Trash2 className="w-4 h-4" />
                    </Button>
                  ...
                  <div className="max-w-4xl mx-auto p-8 md:p-12">
                      ...
                      <div className="space-y-4 flex-1">
                        ...
                        <div className="flex items-center gap-4">
                          <Avatar className="h-12 w-12 ring-4 ring-muted shadow-lg">
                            <AvatarImage src={selectedEmail.sender?.avatar_url} />
                          ...
                          <div className="flex flex-col">
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-foreground">{selectedEmail.sender?.full_name}</span>
                              <span className="text-xs text-muted-foreground font-medium">&lt;{selectedEmail.sender?.email}&gt;</span>
                            </div>
                            <span className="text-xs text-muted-foreground font-medium">
                              to me, {format(new Date(selectedEmail.created_at), 'MMMM d, yyyy • HH:mm')}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    {selectedEmail.attachment_url && (
                      <div className="mt-10 p-6 rounded-2xl border-2 border-dashed border-muted bg-muted/5">
                        <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-4">Attachments (1)</h4>
                        <div className="flex items-center gap-4 p-4 rounded-xl bg-background border shadow-sm group hover:shadow-md transition-all w-fit min-w-[280px]">
                          ...
                          <div className="flex flex-col flex-1 min-w-0">
                            <span className="text-sm font-bold truncate">{selectedEmail.attachment_name}</span>
                            <span className="text-[10px] text-muted-foreground">Click to download attachment</span>
                          </div>
                          <a 
                    ...
                    <div className="flex flex-wrap gap-4">
                       <Button variant="outline" className="rounded-full px-6 h-11 border-2 hover:bg-primary/5 hover:border-primary/50 transition-all font-semibold">
              ...
              <div className="flex flex-col items-center justify-center p-12 text-center animate-in fade-in duration-500">
                ...
                <h3 className="text-2xl font-bold text-foreground">Select an email to read</h3>
                <p className="mt-3 text-muted-foreground max-w-sm leading-relaxed">
                  Choose a message from the list on the left to see its full content here.
                </p>
              </div>
            )}
          </div>
        ...
        <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden rounded-3xl border-none shadow-2xl">
          ...
          <div className="p-6 space-y-4">
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground px-1">Recipient</label>
              <Select onValueChange={setComposeTo} value={composeTo}>
                ...
                <SelectContent className="rounded-xl border-none shadow-xl">
                  {members.filter(m => m.user_id !== user?.id).map((member) => (
                    <SelectItem key={member.id} value={member.user_id} className="rounded-lg">
                      <div className="flex items-center gap-2">
                        <Avatar className="h-6 w-6">
                          <AvatarImage src={member.user_profiles?.avatar_url} />
                          <AvatarFallback>{member.user_profiles?.full_name?.charAt(0)}</AvatarFallback>
                        </Avatar>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground px-1">Subject</label>
              <Input 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground px-1">Message Content</label>
              <textarea 
                rows={8}
            ...
            {composeAttachment && (
              <div className="p-3 rounded-xl bg-primary/5 border border-primary/20 flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
                ...
                <div className="flex flex-col flex-1 min-w-0">
                  <span className="text-xs font-bold truncate">{composeAttachment.name}</span>
                </div>
                <Button 
                  variant="ghost" 
                  size="icon" 
          ...
          <DialogFooter className="px-6 py-4 bg-muted/30 border-t flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button variant="ghost" className="rounded-xl font-semibold" onClick={() => setIsComposeOpen(false)}>
                Discard
              </Button>
              <Button 
